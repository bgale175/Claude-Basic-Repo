...
function judgeMC(btn, wrap, chosenId, correctId) {
  ...
  studyContinue(wrap.closest('.q'), correct ? null : BY_ID.get(correctId).n);
}
...
function renderType(q, id) {
  ...
  const hint = el('div', 'q-hint');
  hint.textContent = 'Type your answer, or press Enter to reveal it';
  const fb = el('div', 'q-feedback');
  // Short arm delay so a stray Enter carried over from the previous card's
  // Continue doesn't instantly reveal this one.
  const armed = performance.now() + 140;
  const submit = () => {
    if (study.locked || performance.now() < armed) return;
    const val = norm(input.value);
    study.locked = true;
    const c = BY_ID.get(id);
    const answer = facetText(c, study.answer);
    const correct = !!val && acceptable(id).includes(val);
    input.classList.add(correct ? 'correct' : 'wrong');
    input.disabled = true;
    hint.remove();
    fb.innerHTML = correct
      ? `<span class="ok">Correct!</span> — <b>${answer}</b>`
      : val
        ? `<span class="no">Not quite</span> — the answer is <b>${answer}</b>`
        : `<span class="no">Answer:</span> <b>${answer}</b>`;
    sfx(correct ? 'good' : 'bad');
    recordAnswer(id, correct);
    studyContinue(q, null);
  };
...
/**
 * After an answer, wait for the player to move on themselves — no timer — so
 * they can sit and read the answer for as long as they like. Enter, Space, or
 * the Continue button advances.
 */
function studyContinue(qEl, note) {
  const bar = el('div', 'q-actions');
  if (note) {
    const n = el('div', 'q-note');
    n.innerHTML = `The answer is <b>${note}</b>`;
    bar.appendChild(n);
  }
  const btn = el('button', 'q-btn');
  btn.innerHTML = 'Continue <span class="kbd">↵</span>';
  const go = () => { if (!study._continue) return; study._continue = null; nextTask(); };
  btn.onclick = go;
  study._continue = go;
  bar.appendChild(btn);
  qEl.appendChild(bar);
